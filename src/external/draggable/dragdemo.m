...
%
%   'group'         Four circles are drawn; draggable's "motionfcn" 
%                   argument is used to make them all move as a group 
%                   instead of individually.
%   
%   'dial'          A crude dial is drawn; draggable's "data" constraint
%                   is used to make the thumb hole follow a path around.

% (C) Copyright 2004-2023
% François Bouffard
% fbouffard@gmail.com
...
switch lower(demotitle)
    ...
    case 'dragtest'
        ...
        %   The Gray Text demonstrates diagonal constraint operating on
        ...
        % The marker on the noisy line demonstrates the data constraint 
        % operating on a single plot marker. On purppose, the marker
        % doesn't start on the data. It should instantly snap to it when
        % dragged.
        
        lims = xlim;
        xn = linspace(lims(1),lims(2),100);
        yn = 0.25 + 0.05*randn(1,100);
        plot(xn,yn,'Color','k');
        marker = plot(0.5,0.5,'ro','MarkerSize',8);

        draggable(marker,'data',[xn;yn]);
    ...
    case 'group'
        ...
        % GROUP demo
        %   Creates a group of three objects that are all moved when one of
        %   them is dragged.
        
        figure;
        axis equal; box on;
        set(gca,'DataAspectRatio',[1 1 1],'Xlim',[-2 2],'YLim',[-2 2]);
        title('GROUP demo for draggable.m')
        ...
        % Saving the handles of every vertices in all vertices, along
        % with their initial position, and its index
        for k = 1:length(vv)
            setappdata(vv(k),'group',vv);
            setappdata(vv(k),'init',[-1 0; 0 -1; 1 0; 0 1]);
            setappdata(vv(k),'index',k);
        end
        
        % Calling draggable with the move_group motionfcn.
        draggable(vv,@move_group);
        
     case 'follow'
        
        % FOLLOW demo
        %   Building the cross, using plot() with a NaN y-element so
        %   that it is non-continuous. The cross spans 0.2 units:
        ...
        grid on;
        box on;
        plot(rand(1,100),rand(1,100),'.b') % background stuff
        p = plot([0.5 0.5 0.4 0.4 0.6],[0.4 0.6 NaN 0.5 0.5]);
        ...
        axis equal;
        set(gca,'DataAspectRatio',[1 1 1],'Xlim',[0 1],'YLim',[0 1]);
        title('FOLLOW demo for draggable.m');
        
        %   Call to draggable(), using no movement constraint and limits
        %   corresponding to the axes limits, plus half the size of the
        %   cross, so that its center always stays in the axes:
        
        draggable(p,'n',[-inf inf -inf inf],@set_lims); 

    case 'dial'

        % DIAL demo
        %   Simulating a rotating dial using the data constraint.
        ...
        xlim([-1 1]);
        ylim([-1 1]);
        title('DIAL demo for draggable.m')
        ...
        % Making invisible fingerhole "track" serving as constraint
        tr = 0.5;
        tx = tr*cos(theta);
        ty = tr*sin(theta);
        % Drawing the fingerhole
        fingerhole = plot(tx(1),ty(1),'bo','MarkerSize',15,'LineWidth',2);
        
        % Making the fingerhole draggable
        draggable(fingerhole,'data',[tx; ty]);
...
% -----------------------------------------------------------------------
% Function MOVE_GROUP
%   This updates the position of every members of the group

function move_group(v)
vv = getappdata(v,'group');
init = getappdata(v,'init');
index = getappdata(v,'index');
initv = init(index,:);
newx = get(v,'XData');
newy = get(v,'YData');
delta = [newx newy] - initv;
for k = 1:size(init,1)
    set(vv(k),'XData',init(k,1)+delta(1));
    set(vv(k),'YData',init(k,2)+delta(2));
end

% -----------------------------------------------------------------------
% Function SET_LIMS
%   Readjusts the plot limits on the fly
...
dy = cy(2)-cy(1);
% trigger
cross_size = max(cross_xdata) - min(cross_xdata);
prox_trigger = 0.75*cross_size;
flo = 0.1;
fhi = 1-flo;
if cx(2)-x < prox_trigger
    % close to the right border
    xlim([x-fhi*dx x+flo*dx]);
end
if x-cx(1) < prox_trigger
    % close to the left border
    xlim([x-flo*dx x+fhi*dx]);
end
if cy(2)-y < prox_trigger
    % close to top border
    ylim([y-fhi*dy y+flo*dy]);
end
if y-cy(1) < prox_trigger
    % close to bottom border
    ylim([y-flo*dy y+fhi*dy]);
end
