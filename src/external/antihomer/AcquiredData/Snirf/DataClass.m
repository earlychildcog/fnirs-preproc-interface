    ...
    methods
        ...
        function err = LoadHdf5(obj, fileobj, location)
            ...
            try
                ...
                fieldnames = string({h5info(H5F.get_name(fid), [location '/measurementList1']).Datasets.Name});
                nMSL = H5G.get_info(gid).nlinks-2;
                ii = 1;
                for ii = 1:nMSL
                    if ii > length(obj.measurementList)
                        obj.measurementList(ii) = MeasListClass;
                    end
                    flagMSL = obj.measurementList(ii).LoadHdf5(fileobj, [location, '/measurementList', num2str(ii)],fieldnames);
                    if flagMSL < 0
