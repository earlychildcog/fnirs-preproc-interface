% SYNTAX:
% data_dod = hmrR_MotionCorrectWavelet_parallel(data_dod, mlActMan, mlActAuto, iqr, turnon)
%
% UI NAME:
...
%
function data_dod = hmrR_MotionCorrectWavelet_parallel(data_dod, mlActMan, mlActAuto, iqr, turnon)
...
%     if isdeployed()
        db2path = fullfile(fileparts(which(mfilename)), '../Install','db2.mat');
%     else
%         p = ffpath2('db2.mat');
%         db2path = [p, '/db2.mat'];    % extremely slow and unnecessary
%     end
        
    fprintf('Loading %s\n', db2path);
    load(db2path,'db2');  % Load a wavelet (db2 in this case)
        
    qmfilter = qmf(db2,4); % Quadrature mirror filter used for analysis
    L = 4;  % Lowest wavelet scale used in the analysis
    
    parfor ii = 1:length(MeasListAct)
        if MeasListAct(ii) == 1
%             t0 = datetime;
...
%             ln = sprintf("time spent on channel %d %.3f\n", ii, seconds(datetime-t0));
%             fid = fopen(sprintf("logs/%.4d.txt",ii), "w");
%             fprintf(fid, ln);
        end
    end
